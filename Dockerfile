...
CMD /usr/sbin/nginx -c /etc/nginx/nginx.conf && /usr/local/bin/consul-template -consul=$CONSUL:8500 -template="/etc/nginx/nginx.conf.ctmpl:/etc/nginx/nginx.conf:nginx -t && nginx -s reload || true" || true
